...
def read_graphs(stream, format = None,
  ...
  if format in {"amr", "camr"}:
    ...
    generator = codec.case.read(stream, text=text, node_centric=node_centric)
  elif format == "pmb":
...
def main():
  ...
  if arguments.write is not None and \
     ...
     {"dot", "tikz", "displacy", "evaluation", "id", "json", "mrp",
      "source", "targets", "txt", "ucca", "norec", "ace", "case"}:
  ...
  if arguments.write == "case":
    case_graphs = [codec.case.write(graph, graph.input, node_centric=arguments.node_centric) for graph in graphs]
    json.dump(case_graphs, arguments.output, indent=None, ensure_ascii=False)
