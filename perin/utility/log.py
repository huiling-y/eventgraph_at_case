...
class Log:
    ...
    def log_evaluation(self, scores, mode, epoch):
        ...
        f1_score = (scores['en'][0][-1][-1] + scores['es'][0][-1][-1] + scores['pr'][0][-1][-1]) / 3
        if self.log_wandb:

            metrics = ['precision', 'recall', 'f1']
            tags = ['all', 'trigger', 'participant', 'organizer', 'target', 'fname', 'etime', 'place']
            scores = {f"train/{k}/{pairs[i][0]}/{metrics[j]}":pairs[i][1][j] for k,pairs in scores.items() for i in range(len(pairs)) for j in range(3)}
